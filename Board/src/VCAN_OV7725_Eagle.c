 ...
 *  @since      v5.0
 */
void ov7725_eagle_port_init()                   //鹰眼管脚初始化
{
    ...
    DMA_DIS(CAMERA_DMA_CH);                         //禁止DMA通道传输
    disable_irq(PORTA_IRQn);                        //关闭PTA的中断
    DMA_IRQ_CLEAN(CAMERA_DMA_CH);                   //清除通道传输中断标志位
    DMA_IRQ_EN(CAMERA_DMA_CH);                      //允许 DMA通道传输
...
#if 1
        PORTA_ISFR = 1 <<  PT27;            //清空PCLK标志位 PORTA_ISFR = 1 <<  PT27;
...
{
    //寄存器，寄存器值次
    {OV7725_COM4         , 0x81},                     //50  0xC1   75  0x41    112  0x81     150  0xC1
